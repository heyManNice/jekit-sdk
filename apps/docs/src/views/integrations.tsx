    ...
    const items: IntegrationItem[] = [
        {
            ...
            snippet: (
                <>
                    ...
                    <span className="text-syntax-identifier">script</span>
                    <span className="text-syntax-string"> src</span>
                    <span className="text-syntax-muted">=</span>
                    <span className="text-syntax-string">"https://..." async</span>
                    <span className="text-syntax-muted">{'></'}</span>
                    <span className="text-syntax-identifier">script</span>
        ...
        },
        {
            ...
            snippet: (
                <>
                    ...
                    <span className="text-syntax-identifier">useJekit</span>
                    <span className="text-syntax-punctuation"> {'}'} </span>
                    <span className="text-syntax-identifier">from</span>
                    <span className="text-syntax-string"> "jekit-vue"</span>
                    <span className="text-syntax-text">;</span>
                </>
            )
        },
        {
            ...
            snippet: (
                <>
                    ...
                    <span className="text-syntax-string"> "jekit-react"</span>
                    <span className="text-syntax-text">;</span>
                </>
            )
        }
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                {items.map((item) => (
                    <Link key={item.title} to={item.href} aria-label={item.ariaLabel} className="group">
                        <GlowCard className="rounded overflow-hidden bg-surface-subtle/50 p-3 flex flex-col gap-2 select-none">
                            <div className="flex items-center gap-5 text-sm font-medium text-text-heading">
                                <div className="flex p-2 items-center justify-center rounded bg-primary-soft/5">
                                    {item.icon}
                                </div>
                                <div>
                                    <div>{item.title}</div>
                            ...
                            <pre tabIndex={-1} className="mt-3 text-sm bg-code-panel/30 p-1 rounded text-primary-soft overflow-x-auto select-text">
                                {item.snippet}
                            </pre>
                        </GlowCard>
                    </Link>
